...
## P1-14 — MCP servers and plugins are never measured — `done`
...
**Acceptance.** Installed plugin skills appear in `scan` with their resident
cost, and `.mcp.json` is discovered.

Measured on a real machine, plugin skill descriptions came to 15,649 resident
tokens across 130 skills, against 565 tokens of always-on instructions. The
tool schemas those servers expose are a separate problem, recorded as P1-16,
because they exist only in a running server.
...
## P1-16 — MCP tool schemas are still unmeasured — `open`

**Problem.** `.mcp.json` names the servers but not the tools they expose. The
schemas are generated by the running server and are frequently the largest
resident block in a session, so the resident total still understates a machine
with servers configured.

**Change.** Either start each configured server and count the schemas it
advertises, or read a cached listing if the agent writes one. Both are heavier
than anything the scanner does today and need a decision about whether a static
audit may execute processes.
